...
%% revised code
% [meanMin1, meanTop, meanMin2,mediantmp,meantmp] = GetMeanMinAndMeanTop_v2(adjcMatrix, colDistM, 0.01);
% clipVal = meanMin2;
% 
% % Emperically choose adaptive sigma for converting geodesic distance to
% % weight
% geoSigma = min([(mediantmp+meantmp)/2, meanMin1 * 3, meanTop / 10]);
% geoSigma = max(geoSigma, (mediantmp+meantmp)/4);
% 
% % Emperically choose adaptive sigma for smoothness term in Equa(9) of our
% % paper.
% neiSigma = min([3 * meanMin1, 0.2 * meanTop, mediantmp+meantmp]);